...
class Font_Awesome {
	 ...
	 * @var string
	 */
	public $version = AYECODE_FA_VERSION;
	...
	/**
	 * Font_Awesome instance.
	 *
	 * @access private
	 * @since  1.0.0
	 * @var    Font_Awesome There can be only one!
	 */
	private static $instance = null;

	/**
	 * Main Font_Awesome Instance.
	 *
	 * Ensures only one instance of Font_Awesome is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Font_Awesome - Main instance.
	 */
	public static function instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Font_Awesome ) ) {
			self::$instance = new Font_Awesome;
		}
	...
	public function init() {
		...
		if ( is_admin() && ! $this->settings_framework ) {
			require_once AYECODE_FA_PLUGIN_DIR . 'src/Settings.php';
	...
	public function add_generator() {
		$file        = str_replace( array( "/", "\\" ), "/", realpath( AYECODE_FA_PLUGIN_FILE ) );
	...
	public function register_custom_icons_library( $libraries ) {
		...
		if ( empty( $local_icon_version ) || empty( $local_icon_styles ) ) {
			$plugin_url  = plugin_dir_url( AYECODE_FA_PLUGIN_FILE );
	...
	public function rest_fa_search( $request ) {
		...
		// Get auth token.
		$icon_gen   = Icon_Library_Generator::instance();
